...
# =========== 抽選関数 ============
    # 抽選(通常)
    @staticmethod
    def lottery(probability=1/349, big_bonus_probability=0.2):
        user = random.random()
        if user <= probability:
    ...
    # 抽選(確変)
    @staticmethod
    def rush_lottery(probability_of_winning=1/30, probability_of_losing=1/180, big_bonus_probability=0.8):
        user = random.random()
...
# =========== 北斗の拳のループ ============
    ...
    def lottery_flow(self, jackpot=None):
        ...
        while ((Pachinko.replay == 0) and (self.balls > 0)) or ((Pachinko.replay != 0) and (Pachinko.balls_for_replay > 0)):
            ...
            if goes_in is False:
                ...
                jackpot = Hokuto.lottery()  # jackpot >>> False:ハズレ、int:当選、
                # 当たり判定
                if jackpot is False:
                    continue
                else:
    ...
    # 確変時の抽選フロー
    def _rush_lottery_flow(self, jackpot=None):
        # 転落か当選を引くまでループ
        while True:
            ...
            if goes_in is False:
                ...
                jackpot = Hokuto.rush_lottery()  # jackpot >>> False:ハズレ、int:当選、str:転落
                if jackpot is False:
                    continue
                else:
                    break
    ...
    # 確率変動
    def bonus_time(self, jackpot=None):
        # 転落を引くまでループ
        while True:
            print(f'\n< 確率変動: {Pachinko.total_bonus}回目 >')
            Pachinko.firing_balls(self)
            jackpot = Hokuto._rush_lottery_flow(self)
            # 当選
...
# =========== メイン ============
    def main(self):
        ...
        while (self.money >= Hokuto.min_amount) or (self.balls > 0):
            ...
            if playing is False:
                ...
                if isinstance(jackpot, bool):
                    ...
                    Hokuto.bonus_time(self)  # 確変(転落を引くまでループ)
        ...
        # 持ち玉を使い切った場合
        elif (self.money != 0) and (self.balls == 0):
