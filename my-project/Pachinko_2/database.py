...
class DataBase:
    ...
    # データベース新規作成: TABLE >>> spec(Name, Probability_Normal, Payout_First, Distribution_First,
    #                                  Probability_Rush, Distribution_Rush, Continuation_Probability, Supplement)
    @staticmethod
    def create_spec_db_first():
        with sqlite3.connect('spec.db') as conn:
            cursor = conn.cursor()
    ...
    # スペックデータがデータベースに存在するか確認、なければ追加
    @staticmethod
    def check_and_add_spec_db():
        with sqlite3.connect('spec.db') as conn:
            cursor = conn.cursor()
            for model in DataBase.model_list:
                check = DataBase.check_spec_db(model)  # データベースを確認
                # データベース(specテーブル)が空
                if check is None:
    ...
    # ======================================  pachinko.db(ユーザー情報)  ======================================
    # データベース作成: TABLE >>> user(Name, Age, Money), history(Data, Name, Age, Model, Income)
    @staticmethod
    ...
    # 退店前にuserテーブルの所持金を更新
    @staticmethod
    def update_user_table(name, age, money):
        with sqlite3.connect('pachinko.db') as conn:
            cursor = conn.cursor()
    ...
    # 管理人のデータベース確認
    @staticmethod
    def check_pachinko_db(num):  # num(int) >>> 1:ユーザー情報確認、2:history:遊戯履歴確認
        instruction_dict = {
            1: 'SELECT * FROM user',
        ...
        with sqlite3.connect('pachinko.db') as conn:
            ...
            if num in instruction_dict:
                ...
                if num == 1:
                    DataBase.show_user(itr)
                elif num == 2:
                    DataBase.show_history(itr)
            else:
                print('入力が正しくありません。')
