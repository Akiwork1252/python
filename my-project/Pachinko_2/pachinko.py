...
from hokuto import FistOfTheNorthStar
...
class Pachinko(Store):
    ...
    # 所持金をパチンコ玉に変換(500円 -> 125玉) <<< PlayGamesから実行()
    def lend(self, amount=500, ball=125):
        print(f'\n{amount}円でパチンコ玉{ball}玉に交換できます。交換しますか？', end='')
    ...
    # ヘソ入賞(通常時) 入賞確率7% >>> 1000円(250玉)で約17回転
    ...
    # 遊戯中の情報表示(確変時) >>> 回転数、獲得出玉、総出玉、ボーナス回数
    ...
    # 大当り獲得時の処理(遊戯玉加算、総出玉加算、回転数初期化, csvファイルにデータ保存)
    ...
    # 大当り終了後の処理(回転数初期化、ボーナス数初期化、csvファイルに保存)
    ...
    def revenue(self):
        ...
        DataBase.save_data_to_history_table(self.name, self.age, self.model_played, income_and_expenditure)  # 遊戯履歴保存
        GameData.information(self.name, self.model_played)
        self.model_played = ''  # 台を離れる際に遊戯台を初期化
